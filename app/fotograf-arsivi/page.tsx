...
export default async function Dashboard() {
  ...
  const sonYapilanlar = db.prepare(`
  ...
  // Depoya giren son 5 malzemeyi yeni teknik matris ilişkisiyle çekiyoruz
  const sonMalzemeler = db.prepare(`
  ...
  const toplamMaliyetMetraj = sonYapilanlar.reduce((sum, item) => sum + (item.quantity_length * item.unit_price_tl), 0);
  const depodakiMalzemeSayisi = db.prepare("SELECT COUNT(*) as count FROM malzemeler WHERE situation = 'Depoda'").get() as { count: number };
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <div className="bg-white p-6 rounded-lg border border-neutral-200 shadow-sm">
          ...
          <p className="text-2xl font-bold mt-2 text-neutral-900">{depodakiMalzemeSayisi.count} Kalem</p>
        </div>
        <div className="bg-white p-6 rounded-lg border border-neutral-200 shadow-sm">
            ...
            <table className="w-full text-left text-sm text-neutral-600">
              ...
              <tbody className="divide-y divide-neutral-100">
                {sonMalzemeler.map((item) => (
                  <tr key={item.id} className="hover:bg-neutral-50/50">
                    <td className="py-3 font-medium text-neutral-950 text-xs">{item.type_name}</td>
                    <td className="py-3 text-xs">{item.brand_name}</td>
                    <td className="py-3 text-center">
